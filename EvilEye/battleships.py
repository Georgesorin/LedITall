...
• Each player places exactly 3 ships on their own wall
• Ships are hidden during battle
• Simultaneous rounds
• Press one tile to choose attack position on your own wall
• Re-press same tile to cycle target through alive opponents, then NOTHING
• Stable round timer (no drift)
• Discovery UI before start
• Works with protocol:
    channel/wall = 1..4
    eye LED      = 0
    buttons      = 1..10
...
EYE_LED = 0
BUTTONS = list(range(1, 11))  # protocol buttons are 1..10
SHIPS_PER_PLAYER = 3
...
def discover_devices_on_interface(bind_ip: str, broadcast_ip: str, timeout_s: float = 3.0):
    ...
    try:
        try:
            sock.bind((bind_ip, 0))   # important: do NOT bind discovery to 7800
        except Exception:
            sock.bind(("", 0))
