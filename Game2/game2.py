...
verifica_si_instaleaza('pygame_gui')
...
# --- CULORI TEMATICE CONSOLĂ ---
CLR_GLASS = (30, 30, 50, 150)
CLR_NEON_PINK = (255, 20, 147)

# ════════════════════════════════════════════════════════════════════════
# 1. INTERFAȚA DE CONTROL (CONSOLĂ DJ)
# ════════════════════════════════════════════════════════════════════════
class InterfaceManager:
    def __init__(self):
        ...
        self.scoreboard_process = None # Protecție pentru ferestre multiple
...
class PianoTilesPro:
    ...
    def tick(self):
        with self.lock:
            ...
            if self.state == "PLAYING":
                # FINAL DE MELODIE - Trecem în modul FINISHED (Game Over)
                ...
                # Setăm un grid constant legat de BPM
                grid_interval = self.base_interval / 1.5 
                
                # Viteză de mișcare curată (evită ca pixelii să se încancece pe orizontală)
                ...
                if now - self.spawn_timer > grid_interval:
                    ...
                    # LOGICĂ SMART PENTRU DISTRIBUȚIE
                    should_spawn = False
                    
                    if self.consecutive_pauses >= 2: 
                        # Nu lăsăm mai mult de 2 spații goale (eliminăm pauzele prea lungi)
                        should_spawn = True
                    elif self.consecutive_spawns >= random.choice([2, 3]): 
                        # Dacă am avut 2 sau 3 pixeli lipiți, FORȚĂM o pauză
                        should_spawn = False
                    else:
                        # În rest, 60% șansă normală
    ...
    def render(self):
        ...
        with self.lock:
            ...
            elif self.state == "FINISHED": # Animația de Game Over
                self._render_finished(buf, now)
        return buf

    def _render_finished(self, buf, t):
        """Efect Game Over: Matricea pulsează cu auriu/galben."""
    ...
    def _render_text_long(self, buf, now):
        ...
        pixels_to_draw = int((elapsed / 3.0) * len(pixels_sequence))
        ...
        while True:
            try:
                ...
                if len(data) >= 1370:
                    ...
                    for c in range(8):
                        off = 2 + (c * 171) + 1 
                        for i in range(64): new_st[(c * 64) + i] = (data[off + i] > 0)
...
if __name__ == "__main__":
    ...
    while ui.running:
        ...
        if ui.game_started and game and getattr(game, 'state', '') == "FINISHED":
            ...
            ui.btn_start.set_text("START NEXT TRACK")
            ...
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == ui.btn_start:
                    try:
                        ...
                        # PORNIM SCOREBOARD-UL DOAR DACA NU EXISTA DEJA UNUL
                        ...
                        if res:
                            ...
                            # Reciclăm rețeaua dacă există, altfel o pornim
                            if net is None:
